...
async def main(days):
    ...
    days = min(days, 10)  # Ograniczenie do 10 dni
    tasks = [get_rates((datetime.strptime(TODAY, "%Y-%m-%d") - timedelta(days=i)).strftime("%Y-%m-%d")) for i in range(days)]
    results = await asyncio.gather(*tasks)
    
    output = []
    for i, result in enumerate(results, 1):
        date = (datetime.strptime(TODAY, "%Y-%m-%d") - timedelta(days=i)).strftime("%d.%m.%Y")
        day_data = {date: {}}
        
        if result is not None:  # Sprawdź, czy wynik nie jest None
            for currency in result[0]["rates"]:
                if currency['code'] in ['EUR', 'USD']:  # Sprawdź tylko dla EUR i USD
                    day_data[date][currency['code']] = {
                        'sale': currency['mid'],
                        'purchase': currency['mid']
                    }
